...
AnalyseV5 <- dplyr::select(V5, 3,6,11,14:28) 
...
SortIpV5 <- com_clust(SortIpV5, variables = c(21:43))
SortIpV5 <- SortIpV5[[1]]
```
Plot Kookkurrenz über Situationen hinweg (ohne Lemma und Wortart)
```{r}
SortIpV5 <- com_clust(SortIpV5, variables = c(21:41))
SortIpV5 <- SortIpV5[[1]]
```
...
SortIpV5Ma1 <- com_clust(SortIpV5Ma1,variables = c(21:43))
SortIpV5Ma1 <- SortIpV5Ma1[[1]]
SortIpV5Ma2 <- com_clust(SortIpV5Ma2,variables = c(21:43))
SortIpV5Ma2 <- SortIpV5Ma2[[1]]
SortIpV5Ma4 <- com_clust(SortIpV5Ma4,variables = c(21:43))
...
SortIpV5Ma1 <- com_clust(SortIpV5Ma1,variables = c(21:41))
SortIpV5Ma1 <- SortIpV5Ma1[[1]]
SortIpV5Ma2 <- com_clust(SortIpV5Ma2,variables = c(21:41))
SortIpV5Ma2 <- SortIpV5Ma2[[1]]
SortIpV5Ma4 <- com_clust(SortIpV5Ma4,variables = c(21:41))
SortIpV5Ma4 <- SortIpV5Ma4[[1]]
```
Summarise by group/global (mit Lemma und Wortart)
```{r}
V5Sum <- SortIpV5[,21:44]
...
Boxplot for Groups
```{r}
AnalyseV5[,9:19] %>%  melt(id.vars= "group")%>% group_by(group, variable,value) %>% count() %>% tidyr::unite(Variante, variable,value,sep=":") %>% ggplot(aes(x=group, y= n,fill=factor(Variante)))+geom_bar(stat="identity", position = "dodge")
...
discriminant analysis global (mit Lemma und Wortart)
```{r}
...
priors <- count(V5Sum, group)
priors <- c(priors$n)
V5SumLDA <- lda(V5Sum[,-c(17,18,22)], V5Sum$group, prior=priors/nrow(V5Sum))
V5SumLDA
plot(V5SumLDA)
V5SumLDAScal <- as.data.frame(V5SumLDA$scaling)
V5SumLDAScal[,3] <-V5SumLDAScal$LD1- V5SumLDAScal$LD2
V5SumLDAScal
rownumbers <- row(V5SumLDAScal)
ggplot(data=V5SumLDAScal, aes(x=LD1, y=LD2))+geom_text(aes(label=rownames(V5SumLDAScal)))
```
discriminant analysis global (ohne Lemma und Wortart)
```{r}
#V5Sum <- rename(V5Sum, group= as.factor.group.)
# Vars <- colnames(V5Sum)
# f <- paste(Vars[24], "~", paste(Vars[-c(13,22:24,16:17,35:36)], collapse=" + "))
...
priors <- count(V5Sum, group)
priors <- c(priors$n)
V5SumLDA <- lda(V5Sum[,-c(17,18,22:65)], V5Sum$group, prior=priors/nrow(V5Sum))
V5SumLDA
plot(V5SumLDA)
V5SumLDAScal <- as.data.frame(V5SumLDA$scaling)
V5SumLDAScal[,3] <-V5SumLDAScal$LD1- V5SumLDAScal$LD2
...
ggplot(data=V5SumLDAScal, aes(x=LD1, y=LD2))+geom_text(aes(label=rownames(V5SumLDAScal)))
```
Summarise by group/situation (mit Lemma und Wortart)
```{r}
V5SumMa1 <- SortIpV5Ma1[,21:44]
# for (k in 1:ncol(V5Ma1Sum)) {
#   for (l in 1:nrow(V5Ma1Sum)) {
#     V5Ma1Sum[l,k] <- as.character(V5Ma1Sum[l,k])
#   }
...
```
LDA group/Situation (mit Lemma und Wortart)
```{r}
...
```
LDA Ma1 mit Lemma und Wortart
```{r}
V5SumMa1 <- V5SumMa1[,which(colSums(V5SumMa1)>0)]#Variablen ausschließen, die nie auftreten und durch ihre Konstanz fehlermeldung verursachen
# Vars <- colnames(V5SumMa1)
#f <- paste(Vars[15], "~", paste(Vars[-c(15)], collapse=" + "))
corV5SumMa1 <- cor(V5SumMa1[,-c(16:18)])
priors <- count(V5SumMa1, group)
priors <- c(priors$n)

V5SumMa1LDA <- lda(V5SumMa1[,-which(names(V5SumMa1) =="group")],V5SumMa1$group, prior=priors/nrow(V5SumMa1))
V5SumMa1LDA
plot(V5SumMa1LDA)
V5SumMa1LDAScal <- as.data.frame(V5SumMa1LDA$scaling)
V5SumMa1LDAScal[,3] <-V5SumMa1LDAScal$LD1- V5SumMa1LDAScal$LD2
ggplot(data=V5SumMa1LDAScal, aes(x=LD1, y=LD2))+geom_text(aes(label=rownames(V5SumMa1LDAScal)))
```
LDA Ma1 ohne Lemma und Wortart
```{r}
V5SumMa1 <- V5SumMa1[,which(colSums(V5SumMa1)>0)]#Variablen ausschließen, die nie auftreten und durch ihre Konstanz fehlermeldung verursachen
# Vars <- colnames(V5SumMa1)
#f <- paste(Vars[15], "~", paste(Vars[-c(15)], collapse=" + "))
...
V5SumMa1LDA <- lda(V5SumMa1[,-c(15:38)],V5SumMa1$group, prior=priors/nrow(V5SumMa1))
V5SumMa1LDA
plot(V5SumMa1LDA)
V5SumMa1LDAScal <- as.data.frame(V5SumMa1LDA$scaling)
V5SumMa1LDAScal[,3] <-V5SumMa1LDAScal$LD1- V5SumMa1LDAScal$LD2
ggplot(data=V5SumMa1LDAScal, aes(x=LD1, y=LD2))+geom_text(aes(label=rownames(V5SumMa1LDAScal)))
```
LDA Ma2 mit Lemma und Wortart
```{r}
V5SumMa2 <- V5SumMa2[,which(colSums(V5SumMa2)>0)]

corV5SumMa2 <- cor(V5SumMa2[,-14])
priors <- count(V5SumMa2, group)
priors <- c(priors$n)
V5SumMa2LDA <- lda(V5SumMa2[,-which(names(V5SumMa2) =="group")],V5SumMa2$group, prior=priors/nrow(V5SumMa2))
V5SumMa2LDA
plot(V5SumMa2LDA)
V5SumMa2LDAScal <- as.data.frame(V5SumMa2LDA$scaling)
V5SumMa2LDAScal[,3] <-V5SumMa2LDAScal$LD1- V5SumMa2LDAScal$LD2
ggplot(data=V5SumMa2LDAScal, aes(x=LD1, y=LD2))+geom_text(aes(label=rownames(V5SumMa2LDAScal)))
```
LDA Ma2 ohne Lemma und Wortart
```{r}
V5SumMa2 <- V5SumMa2[,which(colSums(V5SumMa2)>0)]
...
priors <- count(V5SumMa2, group)
priors <- c(priors$n)
V5SumMa2LDA <- lda(V5SumMa2[,-c(16:40)],V5SumMa2$group, prior=priors/nrow(V5SumMa2))
V5SumMa2LDA
plot(V5SumMa2LDA)
V5SumMa2LDAScal <- as.data.frame(V5SumMa2LDA$scaling)
V5SumMa2LDAScal[,3] <-V5SumMa2LDAScal$LD1- V5SumMa2LDAScal$LD2
...
LDA Ma4 mit Lemma und Wortart
```{r}
V5SumMa4 <- V5SumMa4[,which(colSums(V5SumMa4)>0)]
priors <- count(V5SumMa4, group)
priors <- c(priors$n)
V5SumMa4 <- V5SumMa4[,-c(6,7,38)] # konstante Variablen ausschließen
V5SumMa4LDA <- lda(V5SumMa4[,-which(names(V5SumMa4) =="group")],V5SumMa4$group, prior=priors/nrow(V5SumMa4))
V5SumMa4LDA
plot(V5SumMa4LDA)
V5SumMa4LDAScal <- as.data.frame(V5SumMa4LDA$scaling)
V5SumMa4LDAScal[,3] <-V5SumMa4LDAScal$LD1- V5SumMa4LDAScal$LD2
ggplot(data=V5SumMa4LDAScal, aes(x=LD1, y=LD2))+geom_text(aes(label=rownames(V5SumMa4LDAScal)))
```
LDA Ma4 ohne Lemma und Wortart
```{r}
V5SumMa4 <- V5SumMa4[,which(colSums(V5SumMa4)>0)]
priors <- count(V5SumMa4, group)
priors <- c(priors$n)
#V5SumMa4 <- V5SumMa4[,-c(6,7,10:33)] # konstante Variablen ausschließen
V5SumMa4LDA <- lda(V5SumMa4[,-c(6,7,10:38)],V5SumMa4$group, prior=priors/nrow(V5SumMa4))
V5SumMa4LDA
plot(V5SumMa4LDA)
V5SumMa4LDAScal <- as.data.frame(V5SumMa4LDA$scaling)
V5SumMa4LDAScal[,3] <-V5SumMa4LDAScal$LD1- V5SumMa4LDAScal$LD2
...
ExbDateien mit neuer Annotation speichern (Variable und Variante wieder zusammen oder nicht getrennt)
```{r}
Annotated <- tidyr::unite(Annotated, "Variable", Variable,Variante, sep=":")
CorAnn <- left_join(Corpus,Annotated)

write_back_dir(CorAnn,PathExbDir = pathDir, suffix = "_tagged")
```
...
priors <- count(SortIpV5, group)
priors <- c(priors$n)
V5GlobLDA <- lda( SortIpV5[,-c(1:15,32,39)],SortIpV5$group, prior = priors/nrow(SortIpV5))
...
#corV5 <- cor(AnalyseV5[,-c(13,16:17,22:24,35,36)])
#get priors
priors <- count(AnalyseV5, group)
priors <- c(priors$n)
#for k in nrow(priors){}
V5LDA <- lda(AnalyseV5[,-c(3,19)],AnalyseV5$group, prior=priors/nrow(AnalyseV5))
V5LDA
plot(V5LDA)
V5LDAScal <- as.data.frame(V5SumMa4LDA$scaling)
V5LDAScal[,3] <-V5LDAScal$LD1- V5LDAScal$LD2
